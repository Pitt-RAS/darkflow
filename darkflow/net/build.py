...
import json
import os
from ..utils.im_transform_tf import imcv2_recolor as imcv2_recolor_tf
...
class TFNet(object):
    ...
    def add_preprocess_layers(self, inp):
        inp = imcv2_recolor_tf(inp)
        h, w, c = self.meta['inp_size']
        inp = tf.image.resize_images(inp, (h, w), align_corners=True)
        inp = tf.reverse(tf.cast(inp, dtype=tf.float32) * tf.constant(1/255.), axis=[-1])
        return inp
    ...
    def build_forward(self):
        ...
        # Placeholders
        inp_size = [None] + [1080, 1920, 3]#self.meta['inp_size']
        self.inp_train = tf.placeholder(tf.float32, inp_size, 'input')
        self.feed = dict() # other placeholders

        self.inp = self.add_preprocess_layers(self.inp_train)
